...
False. Each of the two fragments is a single stranded stranded segment.
...
They are complementary, so the polarity of the top strand is reversed on the bottom strand. At each position a purine and complementary pyrimidine base will be present.
...
I think the sequencing binding site is where the sequencing primers bind. This is a part of the adapter separate from the oligo-complementary strand, which binds to an oligonucleotide. The primers are added to the binding site so that a polymerase can add new bases to the free 3'-hydroxyl end of the newly synthesized strand.
...
The remain strand is complementary to the original template. It is anchored by its 5' end.
...
No. Read 2 is sequenced off a newly synthesized strand complementary to the strand seqeuenced in read 1.

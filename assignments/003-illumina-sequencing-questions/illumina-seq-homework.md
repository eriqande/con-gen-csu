...
Illumina sequencing works like a PCR.  A DNA polymerase needs a little nub of double stranded DNA to
sit down on to start synthesizing the complementary strand.  In PCRs we can design the primers to fit
our target fragment--in Illumina sequencing, we're given specific sequences that the sequencer is using
to try to amplify (i.e. sequence) our target DNA insert.  
...
No--sequences the complementary strand.